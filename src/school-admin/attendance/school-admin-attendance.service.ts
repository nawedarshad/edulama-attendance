...
export class SchoolAdminAttendanceService {
    ...
    async getDailyAttendance(schoolId: number, dateStr: string, allowHoliday = false) {
        ...
        return teachers.map(t => {
            ...
            // - Else, status = null (Not Taken).

            let status: AttendanceStatus | 'EXCUSED' | null = null;
            ...
            return {
                ...
                phone: t.personalInfo?.phone || null,
                email: t.personalInfo?.email || null,
    ...
    async getClassComparisonReport(schoolId: number, dto: AttendanceReportFilterDto) {
        ...
        for (const session of stats) {
            ...
            entry.totalStudents += session.attendances.length;
            entry.presentCount += session.attendances.filter(a => a.status === 'PRESENT' || a.status === 'LATE').length;
            entry.absentCount += session.attendances.filter(a => a.status === 'ABSENT').length;
            entry.lateCount += session.attendances.filter(a => a.isLate).length;
            entry.excusedCount += session.attendances.filter(a => a.status === 'EXCUSED').length;
        }
