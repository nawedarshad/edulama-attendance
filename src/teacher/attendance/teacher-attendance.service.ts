...
import { AttendanceConfigService } from '../../attendance-config/attendance-config.service';
    ...
    async takeAttendance(teacherId: number, dto: TakeAttendanceDto) {
        ...
        const holiday = await this.prisma.calendarException.findFirst({
            where: {
                ...
                date: attendanceDate,
                OR: [
                    { classId: null }, // School-wide holiday
    ...
    async getLateStudentsForAttendance(
        ...
        const session = await this.prisma.attendanceSession.findFirst({
            where: {
                ...
                subjectId: null, // Only checking daily attendance for late status
                periodId: null,
            },
            include: {
                attendances: {
        ...
        console.log(`[getLateStudents] Found session ${session.id} with ${session.attendances.length} late records.`);
